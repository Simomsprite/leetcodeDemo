...
public class FloydAlgorithm {
    public static void main(String[] args) {
        ...
        //创建 Graph 对象
        Graphs graph = new Graphs(vertex.length, matrix, vertex);
        //调用弗洛伊德算法
        graph.floyd();
        graph.show();
    }
...
class Graphs{
    ...
    /**
     * @param: length 大小  matrix 邻接矩阵  vertex 顶点数组
     * @return:
     */
    //构造器
        ...
        for (int k=0;k<dis.length;k++){
            ...
            for (int i=0;i<dis.length;i++){
                ...
                for (int j=0;j<dis.length;j++){
                    len=dis[i][k]+dis[k][j];//输出从i顶点出发，经过K中间顶点，到达j顶点距离
